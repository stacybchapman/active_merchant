module ActiveMerchant #:nodoc:
  module Billing #:nodoc:
    class CardConnectGateway < Gateway
      ...
      def add_3DS(post, amount, options)
      ...
      def url(action)
        if test?
          test_url + ACTIONS[action]
        else
          (@options[:domain] ? @options[:domain] : live_url) + ACTIONS[action]
        end
      end
      ...
      def commit(action, parameters)
        ...
        url = url(action) 
        response = parse(ssl_request(:put, url, post_data(action, parameters), headers))
      ...
      def error_code_from(response)
        unless success_from(response)
          STANDARD_ERROR_CODE_MAPPING[response["respcode"]]
        end
      end
    end
  end
